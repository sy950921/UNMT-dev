...
class MultiDataset(TorchtextDataset):
    """Contain data and process it.
    ...
    def __init__(self, fields, src_lang, tgt_lang, readers, data, dirs, sort_key,
                 filter_pred=None):
        self.sort_key = sort_key
        can_copy = 'src_map' in fields and 'alignment' in fields

        direction = "%s-%s" % (src_lang, tgt_lang)
        read_iters = [r.read(dat[1], dat[0], direction, dir_) for r, dat, dir_
                      in zip(readers, data, dirs)]
        ...
        for ex_dict in starmap(_join_dicts, zip(*read_iters)):
            ...
            #              k in ex_dict}
            ex_fields = {}
            direction = ex_dict["direction"]
            indices = fields[direction]
            for k, v in ex_dict.items():
                if k == "direction":
                    ...
                    if k in fields:
                        ex_fields[k] = [(k, fields[k])]
            ex_fields["indices"] = indices
            ex_dict.pop("direction")
            ex = Example.fromdict(ex_dict, ex_fields)
            examples.append(ex)
        ...
        super(MultiDataset, self).__init__(examples, fields, filter_pred)
