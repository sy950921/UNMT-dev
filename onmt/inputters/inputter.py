...
def get_multi_fields(
    ...
    """
    Args:
        data_type: type of the input. Options are [text|img|audio].
        langs (list): Language in this translation.
        trans_directions (list): Translation directions of corpus.
        n_feats (int): the number of source features (not counting tokens)
            to create a :class:`torchtext.data.Field` for. (If
            ``src_data_type=="text"``, these fields are stored together
            as a ``TextMultiField``).
        pad (str): Special pad symbol. Used on src and tgt side.
        ...
        dynamic_dict (bool): Whether or not to include source map and
            alignment fields.
        truncate: Cut off src sequences beyond this (passed to
            ``src_data_type``'s data reader - see there for more details).
    ...
    for lang in langs:
        field_kwargs = {"n_feats": nfeatsdict[lang],
                            "include_lengths": True,
                            "pad": pad, "bos": None, "eos": None,
                            "truncate": truncate,
                            "base_name": lang}
        fields[lang] = fields_getters[data_type](**field_kwargs)

    for direct in trans_directions:
        indices = Field(use_vocab=False, dtype=torch.long, sequential=False)
        fields[direct]["indices"] = indices
